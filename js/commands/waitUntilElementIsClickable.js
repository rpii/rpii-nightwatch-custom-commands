...
WaitUntilElementIsClickable.prototype.command = function(element, timeoutInMilliseconds) {
    ...
    this.check(element, function(result, loadedTimeInMilliseconds) {
        if (result) {
            message = '@' + element.name + ' was clickable after ' + (loadedTimeInMilliseconds - self.startTimeInMilliseconds) + ' ms.';
        } else {
            message = '@' + element.name + ' was still not clickable after ' + timeoutInMilliseconds + ' ms.';
        }
        self.api.assert.ok(result, message);
        self.emit('complete');
